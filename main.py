...
@app.post("/models/", response_model=schemas.Job, status_code=201)
def create_new_model(model: schemas.JobCreate, db: Session = Depends(get_db)):
...
@app.get("/models/{model_id}", response_model=schemas.Job)
...
@app.put("/models/{model_id}", response_model=schemas.Job)
...
@app.delete("/models/{model_id}", response_model=schemas.Job)
